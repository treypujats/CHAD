...
CovidConfirmedCases <- as.data.frame(read.csv("https://usafactsstatic.blob.core.windows.net/public/data/covid-19/covid_confirmed_usafacts.csv"))
...
AFBaseLocations <- as.data.frame(data.table::fread("https://github.com/treypujats/CHAD/raw/master/data/testdata/AFBaseLocationsTest.csv"))
himd <- as.data.frame(data.table::fread("https://github.com/treypujats/CHAD/blob/master/data/testdata/himdtest.csv?raw=true"))
cimd <- as.data.frame(data.table::fread("https://github.com/treypujats/CHAD/blob/master/data/testdata/cimdtest.csv?raw=true"))
CountyInfo <- as.data.frame(data.table::fread("https://github.com/treypujats/CHAD/raw/master/data/testdata/CountyInfoTest.csv"))
...
CovidDeaths<-CovidDeaths[colSums(!is.na(CovidDeaths)) > 0]
...
###################################################################################################################
###################################################################################################################
#Read in global data, replace any open spaces with NA (Change to zero later maybe)
...
lengthdata<-length(CovidConfirmedCases)+32-4
GlobalCases<-GlobalCases[,c(1,4,3,6,33:lengthdata)] #Need the correct number of days now, otherwise rbind wont line up
GlobalCases[,5][is.na(GlobalCases[,5])] <- 0
GlobalCases<- data.frame(t(apply(GlobalCases[,], 1, zoo::na.locf)))
GlobalCases[, 5:length(GlobalCases)][is.na(GlobalCases[, 5:length(GlobalCases)])] <- 0
colnames(GlobalCases)<-colnames(CovidConfirmedCases)
...
lengthdata<-length(CovidDeaths)+32-4
GlobalDeaths<-GlobalDeaths[,c(1,4,3,6,33:lengthdata)] #Need the correct number of days now, otherwise rbind wont line up
GlobalDeaths[,5][is.na(GlobalDeaths[,5])] <- 0
GlobalDeaths<- data.frame(t(apply(GlobalDeaths[,], 1, zoo::na.locf)))
GlobalDeaths[, 5:length(GlobalDeaths)][is.na(GlobalDeaths[, 5:length(GlobalDeaths)])] <- 0
colnames(GlobalDeaths)<-colnames(CovidDeaths)
...
NationalDataTable<-as.data.frame(data.table::fread("https://usafactsstatic.blob.core.windows.net/public/data/covid-19/covid_confirmed_usafacts.csv"))
NationalDataTable<-NationalDataTable[colSums(!is.na(NationalDataTable)) > 0]
...
NationalDeathTable<-as.data.frame(data.table::fread("https://usafactsstatic.blob.core.windows.net/public/data/covid-19/covid_deaths_usafacts.csv"))
...
HeatMapForecast<-merge(AFBaseLocations[c(1:157),], ForecastDataTable, by.x = "Base", by.y = "Installation")
